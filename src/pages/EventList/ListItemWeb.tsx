...
const ListItem = memo(({ eventData, handleEventChange }: IProps) => {
  return (
    <motion.div
      ...
      <p className="sm:ml-14 text-base text-neutral-700 font-normal py-2 pb-0 truncate ">{eventData.action.name}</p>
      <p className="sm:ml-14 text-base text-neutral-700 font-normal py-2 pb-0 truncate ">{dayjs(eventData.occurredAt).isValid() ? dayjs(eventData.occurredAt).format('MMM D, h:mm') : '-'}</p>
    </motion.div>
  )
